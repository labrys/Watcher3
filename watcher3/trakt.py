...
def sync():
    ...
    min_score = watcher3.CONFIG['Search']['Watchlists']['traktscore']
    length = watcher3.CONFIG['Search']['Watchlists']['traktlength']
    movies = []

    if watcher3.CONFIG['Search']['Watchlists']['traktrss']:
        sync_rss()

    for k, v in watcher3.CONFIG['Search']['Watchlists']['Traktlists'].items():
        if v is False:
            continue
    ...
    for i in movies:
        ...
        try:
            if added['response'] and watcher3.CONFIG['Search']['searchafteradd'] and i['year'] != 'N/A':
...
def sync_rss():
    ...
    try:
        record = json.loads(watcher3.sql.system('trakt_sync_record'))
    except Exception as e:
        record = {}

    for url in watcher3.CONFIG['Search']['Watchlists']['traktrss'].split(','):
            ...
            try:
                ...
                if last_sync >= pub:
                    ...
                    if movie:
                        ...
                        if r['response'] and watcher3.CONFIG['Search']['searchafteradd'] and movie['year'] != 'N/A':
                            searcher.search(movie)
                    else:
    ...
    if watcher3.sql.row_exists('SYSTEM', name='trakt_sync_record'):
        watcher3.sql.update('SYSTEM', 'data', json.dumps(record), 'name', 'trakt_sync_record')
    else:
        watcher3.sql.write('SYSTEM', {'data': json.dumps(record), 'name': 'trakt_sync_record'})
