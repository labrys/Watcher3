...
def check_credentials(username, password):
    ...
    Returns bool
    '''
    l = [(watcher3.CONFIG['Server']['authuser'], watcher3.CONFIG['Server']['authpass'])]

    if watcher3.CONFIG['Server']['adminrequired']:
        l.append((watcher3.CONFIG['Server']['adminuser'], watcher3.CONFIG['Server']['adminpass']))
...
def is_admin(username):
    ...
    Returns bool
    '''
    return True if username == watcher3.CONFIG['Server']['adminuser'] else False
...
class AuthController:
    ...
    def __init__(self):
        self.login_form = Template(filename='templates/login.html', module_directory=watcher3.MAKO_CACHE)

    @cherrypy.expose
    def default(self):
        return self.login_form.render(url_base=watcher3.URL_BASE, uitheme=watcher3.CONFIG['Server']['uitheme'])
    ...
    def login(self, username=None, password=None):
        ...
        if check_credentials(username, password) is False:
            ...
            cherrypy.session[watcher3.SESSION_KEY] = cherrypy.request.login = username
    ...
    def logout(self):
        ...
        username = cherrypy.session.get(watcher3.SESSION_KEY, None)
        cherrypy.session.acquire_lock()
        cherrypy.session[watcher3.SESSION_KEY] = None
        cherrypy.session.release_lock()
        if username:
        ...
        return watcher3.URL_BASE + (LOGIN_URL if watcher3.CONFIG['Server']['authrequired'] else '/')
