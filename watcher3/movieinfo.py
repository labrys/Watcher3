...
import os
import re
from watcher3.helpers import Comparisons, Url
_k = Comparisons._k
...
class TheMovieDatabase:
    ...
    def _search_title(title):
        ...
        url = 'https://api.themoviedb.org/3/search/movie?page=1&include_adult={}&'.format('true' if watcher3.CONFIG['Search']['allowadult'] else 'false')
    ...
    def get_imdbid(tmdbid=None, title=None, year=''):
        ...
        if not tmdbid:
            ...
            url = 'https://api.themoviedb.org/3/search/movie?api_key={}&language=en-US&query={}&year={}&page=1&include_adult={}'.format(_k(b'tmdb'), title, year, 'true' if watcher3.CONFIG['Search']['allowadult'] else 'false')
...
class Poster:
    ...
    folder = os.path.join(watcher3.POSTER_DIR)
