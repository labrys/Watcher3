...
def requires_oauth(func):
    ...
    def decor(*args, **kwargs):
        if not watcher3.CONFIG['Downloader']['Torrent']['PutIO']['oauthtoken']:
...
def test_connection(data):
    ...
    response = Url.open(url_base.format('account/info', watcher3.CONFIG['Downloader']['Torrent']['PutIO']['oauthtoken']))
...
def add_torrent(data):
    ...
    if conf['postprocessingenabled']:
        post_data['callback_url'] = '{}/postprocessing/putio_process?apikey={}'.format(conf['externaladdress'], watcher3.CONFIG['Server']['apikey'])
