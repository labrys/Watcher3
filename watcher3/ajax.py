...
import datetime
import watcher3
from watcher3 import config, library, searchresults, searcher, snatcher, notification, plugins, downloaders
from watcher3.library import Metadata, Manage
from watcher3.movieinfo import TheMovieDatabase, YouTube
from watcher3.providers import torrent, newznab
from watcher3.helpers import Conversions
import backup
...
class Ajax:
    ...
    def library(self, sort_key, sort_direction, limit=50, offset=0, status=None, category=None):
        ...
        return watcher3.sql.get_user_movies(sort_key, sort_direction.upper(), limit, offset, status, category)
    ...
    def library_counters(self, category=None):
        ...
        status_count = watcher3.sql.get_library_count('status', 'category', category)
    ...
    def quick_titles(self):
        return watcher3.sql.quick_titles()
    ...
    def get_search_results(self, imdbid, quality=None):
        ...
        if not results:
            ne = watcher3.scheduler_plugin.task_list['Movie Search'].next_execution
    ...
    def save_settings(self, data):
        ...
        save_data = {}
        for key in data:
            if data[key] != watcher3.CONFIG[key]:
    ...
    def delete_movie_file(self, imdbid):
        ...
        try:
            ...
            if os.path.exists(f):
                os.unlink(f)
            watcher3.sql.update_multiple_values('MOVIES', {'finished_date': None, 'finished_score': None, 'finished_file': None}, 'imdbid', imdbid)
    ...
    def search(self, imdbid):
        ...
        if not movie:
            ...
            status = watcher3.sql.get_movie_details('imdbid', imdbid)['status']

            if success:
                results = watcher3.sql.get_search_results(imdbid, movie['quality'], rejected=True)
    ...
    def mark_bad(self, guid, imdbid, cancel_download=False):
        ...
        sr_orig = watcher3.sql.get_single_search_result('guid', guid)
    ...
    def unmark_bad(self, guid, imdbid):
        ...
        if not watcher3.sql.delete('MARKEDRESULTS', 'guid', guid):
    ...
    def notification_remove(self, index):
        ''' Removes notification from watcher3.notification
    ...
    def update_check(self):
        ...
        response = watcher3.updater.update_check()
        if response['status'] == 'current':
            n = [[{'message': _('No updates available.')}, {'type': 'primary'}]]
            return [response, n]
        else:
            return [response, watcher3.NOTIFICATIONS]
    ...
    def update_server(self, mode):
        ...
        set_true:
            Sets watcher3.UPDATING to True, the browser should then automatically redirect
        ...
        update_now:
            Starts update process:
                ...
                * Sets watcher3.UPDATING to False
        ...
        if mode == 'update_now':
            ...
            active_tasks = [k for k, v in watcher3.scheduler_plugin.task_list.items() if v.running]
            ...
            update_status = watcher3.updater.execute_update()
    ...
    def update_movie_options(self, quality, category, status, language, title, filters, imdbid):
        ...
        if status == 'Automatic':
            if not watcher3.sql.update('MOVIES', 'status', 'Waiting', 'imdbid', imdbid):
                return {'response': False, 'error': Errors.database_write}
            ...
            if not watcher3.sql.update('MOVIES', 'status', 'Disabled', 'imdbid', imdbid):
                return {'response': False, 'error': Errors.database_write}
            else:
    ...
    def get_log_text(self, logfile):
        ...
        with open(os.path.join(watcher3.LOG_DIR, logfile)) as f:
    ...
    def get_plugin_conf(self, folder, conf):
        ...
        Returns string
        '''
        c = os.path.join(watcher3.PLUGIN_DIR, folder, conf)
    ...
    def save_plugin_conf(self, folder, filename, config):
        ...
        conf_file = os.path.join(watcher3.PROG_PATH, watcher3.PLUGIN_DIR, folder, filename)
    ...
    def scan_library_directory(self, directory, minsize, recursive, skipduplicatedirs, maxresults):
        ...
        files = watcher3.library.ImportDirectory.scan_dir(directory, minsize, recursive)
        if files.get('error'):
            yield json.dumps({'error': files['error']})
            raise StopIteration()
        user_movies = watcher3.sql.get_user_movies()
        ...
        for i in success:
            for r in fake_results:
                if r['imdbid'] == i['imdbid']:
                    watcher3.sql.update('MOVIES', 'finished_score', r['score'], 'imdbid', i['imdbid'])
                    break

        watcher3.sql.write_search_results(fake_results)
    ...
    def single_movie_details(self, key, value):
        ...
        Returns dict
        '''
        return watcher3.sql.get_movie_details(key, value)
    ...
    def set_movie_details(self, data):
        ...
        if not watcher3.sql.update_multiple_values('MOVIES', data, 'tmdbid', tmdbid):
            return {'response': False, 'error': Errors.database_write}
        else:
    ...
    def manager_backlog_search(self, movies):
        ...
        movies = [i for i in watcher3.sql.get_user_movies() if i['imdbid'] in ids]
    ...
    def manager_change_quality(self, movies, quality):
        ...
        for i, movie in enumerate(movies):
            if not watcher3.sql.update('MOVIES', 'quality', quality, 'imdbid', movie['imdbid']):
                response = {'response': False, 'error': Errors.database_write, 'imdbid': movie['imdbid'], 'index': i + 1}
            else:
                response = {'response': True, 'index': i + 1}
    ...
    def manager_change_category(self, movies, category):
        ...
        for i, movie in enumerate(movies):
            if not watcher3.sql.update('MOVIES', 'category', category, 'imdbid', movie['imdbid']):
                response = {'response': False, 'error': Errors.database_write, 'imdbid': movie['imdbid'], 'index': i + 1}
            else:
                response = {'response': True, 'index': i + 1}
    ...
    def manager_reset_movies(self, movies):
        ...
        for i, movie in enumerate(movies):
            ...
            imdbid = movie['imdbid']
            if not watcher3.sql.purge_search_results(imdbid):
            ...
            if not watcher3.sql.update_multiple_values('MOVIES', db_reset, 'imdbid', imdbid):
    ...
    def create_backup(self):
        ...
        logging.info('Creating backup of Watcher as {}'.format(os.path.join(watcher3.PROG_PATH, 'watcher.zip')))
        ...
        return {'response': True, 'message': _('Backup created as {}').format(os.path.join(watcher3.PROG_PATH, 'watcher.zip'))}
    ...
    def restore_backup(self, fileUpload):
        ...
        tmp_zip = os.path.join(watcher3.PROG_PATH, f'restore_{n}.zip')
        ...
        threading.Timer(3, watcher3.restart).start()
    ...
    def manual_task_execute(self, name):
        ...
        Response includes watcher3.NOTIFICATIONS so the browser can display any
        ...
        try:
            ...
            task = watcher3.scheduler_plugin.task_list[name]
            task.now()

            le = task.last_execution

            return {'response': True, 'message': _('Finished task {}.').format(name), 'last_execution': le, 'notifications': watcher3.NOTIFICATIONS}
        except Exception as e:
            return {'response': False, 'error': str(e)}
