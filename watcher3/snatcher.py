...
import urllib.parse
import watcher3
from watcher3 import plugins
from watcher3 import downloaders
...
def grab_all():
    ...
    keepsearching = watcher3.CONFIG['Search']['keepsearching']
    keepsearchingscore = watcher3.CONFIG['Search']['keepsearchingscore']
    keepsearchingdays = watcher3.CONFIG['Search']['keepsearchingdays']
...
def get_best_release(movie, minscore=0, ignore_guid=None):
    ...
    search_results = watcher3.sql.get_search_results(imdbid, quality)
    ...
    wait_days = watcher3.CONFIG['Search']['waitdays']
    ...
    if (today - date_found).days < wait_days:
        if watcher3.CONFIG['Search']['skipwait'] and release_weeks_old > watcher3.CONFIG['Search']['skipwaitweeks']:
    ...
    if data['type'] in ('torrent', 'magnet'):
        if watcher3.CONFIG['Downloader']['Sources']['torrentenabled']:
            ...
            if not response['response']:
                for result in watcher3.sql.get_single_search_result('guid', data['guid'], all_indexers=True):
...
def snatch_nzb(data):
    ...
    for client, config in watcher3.CONFIG['Downloader']['Usenet'].items():
        if config['enabled']:
            ...
            if response['response']:
                ...
                watcher3.sql.update_multiple_values('SEARCHRESULTS', db_update, 'guid', guid)
    ...
    if urllib.parse.urlparse(guid).netloc:
        ...
        if not guid_:
            ...
            if magnet:
                ...
                watcher3.sql.update('SEARCHRESULTS', 'torrentfile', magnet, 'guid', guid)

        if guid_:
            watcher3.sql.update('SEARCHRESULTS', 'guid', guid_.lower(), 'guid', guid)
            guid = guid_
        else:
