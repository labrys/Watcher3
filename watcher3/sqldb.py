...
class SQL:
    ...
    def __init__(self):
        ...
        DB_NAME = f'sqlite:///{watcher3.DB_FILE}'
        ...
        try:
            ...
            if not os.path.isfile(watcher3.DB_FILE):
                print(f'Creating database file {watcher3.DB_FILE}')
    ...
    def update_multiple_rows(self, TABLE, values, id_col):
        ...
        TABLE = getattr(watcher3.sql, TABLE)

        watcher3.sql.engine.execute(TABLE.update().where(getattr(TABLE.c, id_col) == sqla.bindparam(id_col_)).values(write), values)
    ...
    def get_search_results(self, imdbid, quality=None, rejected=False):
        ...
        if quality in watcher3.CONFIG['Quality']['Profiles'] and watcher3.CONFIG['Quality']['Profiles'][quality]['prefersmaller']:
            sort = 'ASC'
        else:
            sort = 'DESC'

        if watcher3.CONFIG['Search']['preferredsource'] == '':
            sk = ''
        else:
            sk = ''', CASE type WHEN "nzb" THEN 0
                                ...
                                WHEN "magnet" THEN 1
                    END {}
                '''.format('ASC' if watcher3.CONFIG['Search']['preferredsource'] == 'usenet' else 'DESC')
    ...
    def update_database(self):
        ...
        backup_dir = os.path.join(watcher3.PROG_PATH, 'db')
        logging.debug(f'Backing up database to {backup_dir}.')
        print(f'Backing up database to {backup_dir}.')
        try:
            if not os.path.isdir(backup_dir):
                os.mkdir(backup_dir)
            backup_name = f'{watcher3.DB_FILE}.backup-{datetime.date.today()}'

            shutil.copyfile(watcher3.DB_FILE, os.path.join(backup_dir, backup_name))
        except Exception as e:
...
class DatabaseUpdate:
    ...
    def update_1():
        ...
        for ind, i in enumerate(d):
            ...
            if p and 'poster/' in p:
                watcher3.sql.update('MOVIES', 'poster', p.replace('poster/', 'posters/'), 'imdbid', i['imdbid'])
        print()
    ...
    def update_2():
        ...
        if values:
            watcher3.sql.update_multiple_rows('MOVIES', values, 'imdbid')
        print()
    ...
    def update_3():
        ...
        d = watcher3.sql.dump('SEARCHRESULTS')
        l = len(d)

        for ind, i in enumerate(d):
            print(f'{int((ind + 1) / l * 100)}%\r', end='')
            if not i.get('imdbid') and i.get('guid'):
                watcher3.sql.delete('SEARCHRESULTS', 'guid', i['guid'])
        print()
    ...
    def update_6():
        ...
        watcher3.sql.update_tables()
        watcher3.sql.write('SYSTEM', {'name': 'imdb_sync_record', 'data': '{}'})
    ...
    def update_7():
        ...
        if values:
            watcher3.sql.update_multiple_rows('SEARCHRESULTS', values, 'guid')
        print()
    ...
    def update_10():
        ...
        watcher3.sql.update_tables()
        watcher3.sql.update_all('MOVIES', {'category': 'Default'})
