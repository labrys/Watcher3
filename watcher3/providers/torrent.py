...
from watcher3.providers.base import NewzNabProvider
from watcher3.providers import torrent_modules  # noqa
import logging
...
class Torrent(NewzNabProvider):
    ...
    def search_all(self, imdbid, title, year, ignore_if_imdbid_cap = False):
        ...
        torz_indexers = watcher3.CONFIG['Indexers']['TorzNab'].values()
        ...
        for indexer in torz_indexers:
            ...
            caps = watcher3.sql.torznab_caps(url_base)
            if not caps:
        ...
        for indexer, settings in watcher3.CONFIG['Indexers']['Torrent'].items():
            if settings['enabled']:
        ...
        for indexer, indexerobject in watcher3.CONFIG['Indexers']['PrivateTorrent'].items():
    ...
    def _get_caps(self, url_base, apikey):
        ...
        try:
            ...
            watcher3.sql.write('CAPS', {'url': url_base, 'caps': caps})
