...
def base_url():
    url = watcher3.CONFIG['Indexers']['Torrent']['yts']['url']
    if not url:
        url = 'https://yts.mx'
    elif url[-1] == '/':
        url = url[:-1]
...
def _parse(movie, imdbid, title):
    ...
    for i in movie['torrents']:
        ...
        try:
            ...
            result['torrentfile'] = watcher3.providers.torrent.magnet(i['hash'], result['title'])
            result['guid'] = i['hash']
            result['type'] = 'magnet'
...
def _parse_rss(xml):
    ...
    for i in items:
        ...
        try:
            ...
            result['torrentfile'] = watcher3.providers.torrent.magnet(result['guid'], result['title'])
            result['type'] = 'magnet'
            result['downloadid'] = None
            result['seeders'] = 5
            result['leechers'] = 0
