...
def create_plugin():
    ...
    watcher3.scheduler_plugin = SchedulerPlugin(cherrypy.engine, record_handler=record_handler)
    AutoSearch.create()
    ...
    watcher3.scheduler_plugin.subscribe()


class record_handler:
    @staticmethod
    def read():
        return {i['name']: {'last_execution': i['last_execution']} for i in watcher3.sql.dump('TASKS')}

    @staticmethod
    def write(name, le):
        if watcher3.sql.row_exists('TASKS', name=name):
            watcher3.sql.update('TASKS', 'last_execution', le, 'name', name)
        else:
            watcher3.sql.write('TASKS', {'last_execution': le, 'name': name})
        return
...
class PostProcessingScan:
    ...
    @staticmethod
    def create():
        conf = watcher3.CONFIG['Postprocessing']['Scanner']
    ...
    def scan_directory():
        ...
        minsize = watcher3.CONFIG['Postprocessing']['Scanner']['minsize'] * 1048576

        # paths processed with api requests since last task run
        postprocessed_paths = watcher3.sql.get_postprocessed_paths()
        logging.debug(f'Paths already post processed: {postprocessed_paths}')

        files = []
        if conf['newfilesonly']:
            t = watcher3.scheduler_plugin.record.get('PostProcessing Scan', {}).get('last_execution')
            if not t:
        ...
        for i in files:
            ...
            r = watcher3.sql.get_single_search_result('title', fname)
            if r:
            ...
            d = {'apikey': watcher3.CONFIG['Server']['apikey'],
                 'mode': 'complete',
                 'path': i,
                 'guid': r.get('guid') or '',
                 'downloadid': '',
        ...
        watcher3.sql.delete_postprocessed_paths()
...
class AutoUpdateCheck:
    ...
    def create():
        ...
        now = datetime.datetime.today()
        hr = now.hour
        min = now.minute + (watcher3.CONFIG['Server']['checkupdatefrequency'] * 60)
        if now.second > 30:
            min += 1
    ...
    def update_check(install=True):
        ...
        Returns dict from watcher3.updater.update_check():
            {'status': 'error', 'error': <error> }
            {'status': 'behind', 'behind_count': #, 'local_hash': 'abcdefg', 'new_hash': 'bcdefgh'}
            {'status': 'current'}
        '''

        return watcher3.updater.update_check(install=install)
...
class ImdbRssSync:
    ...
    def create():
        ...
        if watcher3.CONFIG['Search']['Watchlists']['imdbsync']:
            auto_start = True
        else:
            auto_start = False
...
class PopularMoviesSync:
    ...
    def create():
        ...
        if watcher3.CONFIG['Search']['Watchlists']['popularmoviessync']:
            auto_start = True
        else:
            auto_start = False
...
class TraktSync:
    ...
    def create():
        ...
        if watcher3.CONFIG['Search']['Watchlists']['traktsync']:
            if any(watcher3.CONFIG['Search']['Watchlists']['Traktlists'].keys()):
                auto_start = True
            else:
    ...
    def scan_paths():
        ...
        for path in postprocessed_paths:
            if not os.path.exists(path):
                watcher3.sql.delete('POSTPROCESSED_PATHS', 'path', path)
...
class FinishedTorrentsCheck:
    ...
    @staticmethod
    def check_torrents():
        for client, config in watcher3.CONFIG['Downloader']['Torrent'].items():
            if config['enabled']:
                progress = {}
                now = int(datetime.datetime.timestamp(datetime.datetime.now()))
                if config.get('removestalledfor'):
                    progress = watcher3.sql.get_download_progress(client)
                ...
                for torrent in downloader.get_torrents_status(stalled_for=config.get('removestalledfor'), progress=progress):
                    ...
                    if torrent['status'] == 'finished' and config.get('removetorrents'):
                        ...
                        if watcher3.sql.row_exists('MARKEDRESULTS', guid=str(torrent['hash']), status='Finished'):
                    ...
                    if torrent['status'] == 'stalled':
                        ...
                        if torrent['hash'] in progress:
                            result = watcher3.sql.get_single_search_result('downloadid', str(torrent['hash']))
                            movie = watcher3.sql.get_movie_details('imdbid', result['imdbid'])
                    ...
                    if progress_update and watcher3.sql.row_exists('SEARCHRESULTS', downloadid=str(torrent['hash']), status='Snatched'):
                        watcher3.sql.update_multiple_values('SEARCHRESULTS', progress_update, 'downloadid', torrent['hash'])
