...
import csv
import threading
from watcher3 import searchresults, plugins
import watcher3
from watcher3.movieinfo import TheMovieDatabase, Poster
...
class ImportKodiLibrary:
    ...
    def get_movies(url):
        ...
        library = [i['imdbid'] for i in watcher3.sql.get_user_movies()]
        movies = []
...
class ImportPlexLibrary:
    ...
    def read_csv(csv_text):
        ...
        library = [i['imdbid'] for i in watcher3.sql.get_user_movies()]
        library_tmdb = [i['tmdbid'] for i in watcher3.sql.get_user_movies()]
...
class Metadata:
    ...
    def parse_filename(filepath):
        ...
        for source, aliases in watcher3.CONFIG['Quality']['Aliases'].items():
    ...
    def get_category_from_path(filepath):
        moverpath = watcher3.CONFIG['Postprocessing']['moverpath']
        if moverpath and filepath.startswith(Metadata.root_mover_path(moverpath)):
            return 'Default'
        for category, category_config in watcher3.CONFIG['Categories'].items():
    ...
    def convert_to_db(movie):
        ...
        movie = {k: v for k, v in movie.items() if k in [i.name for i in watcher3.sql.MOVIES.c]}
    ...
    def tmdb_data(imdbid, tmdbid=None, language=None):
        ...
        if tmdbid is None:
            tmdbid = watcher3.sql.get_movie_details('imdbid', imdbid).get('tmdbid')
            if not tmdbid:
    ...
    def update(imdbid, tmdbid=None, force_poster=True):
        ...
        if force_poster:
            ...
            get_poster = True
        elif not os.path.isfile(os.path.join(watcher3.PROG_PATH, movie['poster'])):
            get_poster = True
        else:
        ...
        watcher3.sql.update_multiple_values('MOVIES', movie, 'imdbid', imdbid)
...
class Manage:
    ...
    def scanmissingfiles():
        movies = watcher3.sql.execute(['SELECT * FROM MOVIES WHERE finished_file is not null'])

        if movies is None:
            logging.warning('Unable to read database.')
            return

        movies = watcher3.sqldb.proxy_to_dict(movies)

        action = watcher3.CONFIG['System']['FileManagement']['missingfileaction']
        db_update_values = []
        for i in movies:
            if not os.path.exists(i['finished_file']):
                ...
                if action == 'remove':
                    ...
                    if watcher3.sql.delete('MARKEDRESULTS', 'imdbid', i['imdbid']) is None or \
                       watcher3.sql.delete('SEARCHRESULTS', 'imdbid', i['imdbid']) is None:
                        logging.warning('Unable to remove marked releases. Status change may be incorrect.')

        if len(db_update_values) > 0:
            watcher3.sql.update_multiple_rows('MOVIES', db_update_values, 'imdbid')
    ...
    def verify(movie, today=None):
        ...
        if watcher3.CONFIG['Search']['verifyreleases'] == '':
            verified = True
            ...
            verified = False
        elif watcher3.CONFIG['Search']['verifyreleasesskip'] and datetime.datetime.strptime(movie['release_date'], '%Y-%m-%d') + datetime.timedelta(days=7 * watcher3.CONFIG['Search']['verifyreleasesskipweeks']) < today:
            logging.info('{} is older than {}, skipping verification check as Verified.'.format(movie['title'], watcher3.CONFIG['Search']['verifyreleasesskipweeks']))
            verified = True

        elif watcher3.CONFIG['Search']['verifyreleases'] == 'predb':
            if movie.get('predb') == 'found':
                verified = True
            else:
                verified = False

        elif watcher3.CONFIG['Search']['verifyreleases'] == 'mediareleasedate':
            if not movie.get('predb') and movie.get('predb_backlog'):
                ...
                watcher3.sql.update('MOVIES', 'predb_backlog', None, 'imdbid', movie['imdbid'])
        ...
        if verified and movie['status'] == 'Waiting':
            ...
            watcher3.sql.update('MOVIES', 'status', 'Wanted', 'imdbid', movie['imdbid'])
            ...
            watcher3.sql.update('MOVIES', 'status', 'Waiting', 'imdbid', movie['imdbid'])
    ...
    def add_movie(movie, full_metadata=False):
        ...
        if watcher3.sql.row_exists('MOVIES', imdbid=movie['imdbid']):
        ...
        if not watcher3.sql.write('MOVIES', movie):
            response['response'] = False
    ...
    def remove_movie(imdbid):
        ...
        Calls watcher3.sql.remove_movie and removes poster (in separate thread)
        ...
        m = watcher3.sql.get_movie_details('imdbid', imdbid)

        removed = watcher3.sql.remove_movie(imdbid)
    ...
    def searchresults(guid, status, movie_info=None):
        ...
        if watcher3.sql.row_exists(TABLE, guid=guid):

            # Mark bad in SEARCHRESULTS
            logging.info('Marking {} as {} in SEARCHRESULTS.'.format(guid.split('&')[0], status))
            if not watcher3.sql.update(TABLE, 'status', status, 'guid', guid):
            ...
            if watcher3.sql.write('SEARCHRESULTS', search_result):
                return True
            else:
                return False
    ...
    def markedresults(guid, status, imdbid=None):
        ...
        if watcher3.sql.row_exists(TABLE, guid=guid):
            ...
            if imdbid:
                ...
                if watcher3.sql.write(TABLE, DB_STRING):
    ...
    def movie_status(imdbid):
        ...
        if watcher3.CONFIG['Downloader']['Sources']['usenetenabled']:
            t.append('nzb')
        ...
        try:
            result_status = [i['status'] for i in watcher3.sql.execute([cmd]).fetchall()] or []
        except Exception as e:
        ...
        if watcher3.sql.update('MOVIES', 'status', new_status, 'imdbid', imdbid):
            return new_status
        else:
    ...
    def add_status_to_search_movies(results):
        ...
        statuses = watcher3.sql.get_movies_status('tmdbid', tmdb_ids)
    ...
    def get_stats(category=None):
        ...
        qualities = {'Default': 0}
        for i in watcher3.CONFIG['Quality']['Profiles']:
            if i == 'Default':
                continue
            qualities[i] = 0
        ...
        if not category:
            ...
            for i in watcher3.CONFIG['Categories']:
                categories[i] = 0
        ...
        movies = watcher3.sql.get_user_movies(category = category)
    ...
    def search_results(imdbid, quality=None):
        ...
        results = watcher3.sql.get_search_results(imdbid, quality=quality, rejected=True)

        if not watcher3.CONFIG['Downloader']['Sources']['usenetenabled']:
            results = [res for res in results if res.get('type') != 'nzb']
        if not watcher3.CONFIG['Downloader']['Sources']['torrentenabled']:
    ...
    def update_movie_options(imdbid, quality, category, language, title, filters):
        ...
        return watcher3.sql.update_multiple_values('MOVIES', new_data, 'imdbid', imdbid)