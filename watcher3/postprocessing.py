...
import cherrypy
import watcher3
from watcher3 import plugins, snatcher
from watcher3.library import Metadata, Manage
from watcher3.downloaders import PutIO
...
class Postprocessing:
    ...
    def putio_process(self, *args, **transfer_data):
        ...
        conf = watcher3.CONFIG['Downloader']['Torrent']['PutIO']
        ...
        if conf['downloadwhencomplete']:
            ...
            if data.get('imdbid'):
                ...
                watcher3.sql.update_multiple_values('MOVIES', db_update, 'imdbid', data['imdbid'])
    ...
    def default(self, **data):
        ...
        **data: keyword params send through POST request payload
        ...
        if data['apikey'] != watcher3.CONFIG['Server']['apikey']:
        ...
        if data['mode'] == 'failed':
            ...
            if 'task' not in data:
                directory = watcher3.CONFIG['Postprocessing']['Scanner']['directory']
                if data['path'] == directory:
                    watcher3.sql.save_postprocessed_path(data['original_file'])
                else:
                    watcher3.sql.save_postprocessed_path(data['path'])
    ...
    def get_movie_info(self, data):
        ...
        if data.get('guid'):
            ...
            result = watcher3.sql.get_single_search_result('guid', data['guid'])
            if result:
        ...
        if not result:  # not found from guid
            ...
            if data.get('downloadid'):
                ...
                result = watcher3.sql.get_single_search_result('downloadid', str(data['downloadid']))
        ...
        if not result:  # not found from guid or downloadid
            ...
            if fname:
                ...
                result = watcher3.sql.get_single_search_result('title', fname)
                if result:
                    ...
                    result = watcher3.sql.get_single_search_result('title', re.sub(r'[\[\]\(\)\-.:]', '_', fname), like=True)
                    if result:
                        logging.info(f'Found match for {fname} in releases.')
                    else:
        ...
        if result:
            ...
            local = watcher3.sql.get_movie_details('imdbid', result['imdbid'])
            if local:
    ...
    def failed(self, data):
        ...
        config = watcher3.CONFIG['Postprocessing']
        ...
        if watcher3.CONFIG['Search']['autograb']:
    ...
    def complete(self, data):
        ...
        If Renamer is enabled, renames movie file according to watcher3.CONFIG
        If Mover is enabled, moves file to location in watcher3.CONFIG, then...
        ...
        if data.get('imdbid'):
            if watcher3.sql.row_exists('MOVIES', imdbid=data['imdbid']):
                data['category'] = watcher3.sql.get_movie_details('imdbid', data['imdbid'])['category']
            else:
        ...
        if data.get('imdbid') and data['imdbid'] != 'N/A':
            watcher3.sql.update('MOVIES', 'finished_file', result['data'].get('finished_file'), 'imdbid', data['imdbid'])
    ...
    def map_remote(self, path):
        ...
        maps = watcher3.CONFIG['Postprocessing']['RemoteMapping']
    ...
    def renamer(self, data):
        ...
        Renames movie file based on params in watcher3.CONFIG
        ...
        if watcher3.CONFIG['Postprocessing']['replacespaces']:
    ...
    def mover(self, data):
        ...
        Moves file to location specified in watcher3.CONFIG
        ...
        if category in watcher3.CONFIG['Categories']:
            moverpath = watcher3.CONFIG['Categories'][category]['moverpath']
        else:
