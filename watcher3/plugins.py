...
def list_plugins():
    ...
    for folder in plugins.keys():
        try:
            p_dir = os.path.join(watcher3.PLUGIN_DIR, folder)
...
def added(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    plugin_dir = os.path.join(watcher3.PROG_PATH, watcher3.PLUGIN_DIR, 'added')

    sorted_plugins = sorted(watcher3.CONFIG['Plugins']['added'].items(), key=operator.itemgetter(1))
...
def snatched(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    plugin_dir = os.path.join(watcher3.PROG_PATH, watcher3.PLUGIN_DIR, 'snatched')

    sorted_plugins = sorted(watcher3.CONFIG['Plugins']['snatched'].items(), key=operator.itemgetter(1))
...
def finished(*args):
    ...
    *args (list): arguments to pass to plugins
    ...
    plugin_dir = os.path.join(watcher3.PROG_PATH, watcher3.PLUGIN_DIR, 'finished')

    sorted_plugins = sorted(watcher3.CONFIG['Plugins']['finished'].items(), key=operator.itemgetter(1))
