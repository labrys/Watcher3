...
def score(releases, imdbid=None, imported=False):
    ...
    if imported:
        ...
        quality = watcher3.CONFIG['Quality']['Profiles'][watcher3.config.default_profile()]
        #quality = import_quality()
   
        category = {'requiredwords': '', 'preferredwords': '', 'ignoredwords': ''}
    else:
        movie_details = watcher3.sql.get_movie_details('imdbid', imdbid)
        ...
        if category_name in watcher3.CONFIG['Categories']:
            category = watcher3.CONFIG['Categories'][category_name]
        else:
            category = {'requiredwords': '', 'preferredwords': '', 'ignoredwords': ''}
    ...
    if movie_details.get('download_language'):
        lang_names = [lang.lower() for lang in watcher3.config.lang_names(movie_details.get('download_language'))]
    ...
    if watcher3.CONFIG['Search']['retention'] > 0 and any(i['type'] == 'nzb' for i in releases):
        releases = retention_check(releases)

    if any(i['type'] in ('torrent', 'magnet') for i in releases):
        if watcher3.CONFIG['Search']['mintorrentseeds'] > 0:
            releases = seed_check(releases)
        if watcher3.CONFIG['Search']['freeleechpoints'] > 0 or watcher3.CONFIG['Search']['requirefreeleech']:
            releases = freeleech(releases)
        if watcher3.CONFIG['Search']['seederspoints'] and watcher3.CONFIG['Search']['seedersthreshold']:
            releases = threshold_score(releases, 'seeders', watcher3.CONFIG['Search']['seederspoints'], watcher3.CONFIG['Search']['seedersthreshold'])
        if watcher3.CONFIG['Search']['leecherspoints'] and watcher3.CONFIG['Search']['leechersthreshold']:
            releases = threshold_score(releases, 'leechers', watcher3.CONFIG['Search']['leecherspoints'], watcher3.CONFIG['Search']['leechersthreshold'])
...
def retention_check(releases):
    ...
    logging.info('Checking retention [threshold = {} days].'.format(watcher3.CONFIG['Search']['retention']))
    reject = 0
    for result in releases:
        ...
        if result['type'] == 'nzb':
            ...
            if age >= watcher3.CONFIG['Search']['retention']:
                logging.debug('{} published {} days ago, removing search result.'.format(result['title'], age))
                result['reject_reason'] = 'older than retention ({})'.format(watcher3.CONFIG['Search']['retention'])
                reject += 1
...
def seed_check(releases):
    ...
    Gets required seeds from watcher3.CONFIG
    ...
    for result in releases:
        ...
        if result['type'] in ('torrent', 'magnet'):
            if int(result['seeders']) < watcher3.CONFIG['Search']['mintorrentseeds']:
                logging.debug('{} has {} seeds, removing search result.'.format(result['title'], result['seeders']))
                result['reject_reason'] = 'not enough seeds ({})'.format(watcher3.CONFIG['Search']['mintorrentseeds'])
                reject += 1
...
def freeleech(releases):
    ...
    points = watcher3.CONFIG['Search']['freeleechpoints']
    reject = 0
...
def score_sources(releases, sources, check_size=True):
    ...
    score_range = len(watcher3.SOURCES) + 1

    sizes = watcher3.CONFIG['Quality']['Sources']
...
def import_quality():
    ...
    Returns dict
    '''
    profile = watcher3.config.base_profile
