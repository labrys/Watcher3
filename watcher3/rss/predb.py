...
def backlog_search(movie):
    ...
    watcher3.sql.update_multiple_values('MOVIES', db_update, 'imdbid', imdbid)
...
def _search_db(title_year):
    ...
    categories = 'movies'
    if watcher3.CONFIG['Search'].get('predb_unknown'):
        categories += ',unknown'
    ...
    try:
        ...
        for movie in movies:
            ...
            if _fuzzy_match(items, title, year):
                ...
                watcher3.sql.update('MOVIES', 'predb', 'found', 'imdbid', imdbid)
                continue
    except Exception as e:
