...
def _t_search_grab(movie):
    ...
    if watcher3.CONFIG['Search']['verifyreleases'] == 'predb':
        movie = predb.backlog_search(movie)

    if not Manage.verify(movie):
        return

    if watcher3.CONFIG['Search']['searchafteradd'] and search(movie) and watcher3.CONFIG['Search']['autograb']:
...
default status Available.
    ...
    old_results = watcher3.sql.get_search_results(imdbid, quality)
    ...
    marked_results = watcher3.sql.get_marked_results(imdbid)
    if marked_results:
    ...
    if not watcher3.sql.update('MOVIES', 'backlog', '1', 'imdbid', imdbid):
...
def rss_sync(movies):
    ...
    for movie in movies:
        ...
        old_results = watcher3.sql.get_search_results(imdbid, rejected=True)
        new_results = []
        for res in results:
            guid = res['guid']
...
def remove_inactive(results):
    ...
    active = []
    for i in watcher3.CONFIG['Indexers']['NewzNab'].values():
        if i[2] is True:
            active.append(i[0])
...
default False>
    ...
    if backlog:
        ...
        watcher3.sql.purge_search_results(imdbid=imdbid)

    if BATCH_DB_STRING:
        if watcher3.sql.write_search_results(BATCH_DB_STRING):
            return True
        else:
            return False
    else:
...
def get_source(ptn_data):
    ...
    Returns str source based on watcher3.SOURCES
    '''
