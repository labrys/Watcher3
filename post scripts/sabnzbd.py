...
url = f'http://{sabhost}:{sabport}/sabnzbd/api?apikey={sabkey}&mode=history&output=json&search={name}'
...
if status == 0:
    print(f'Sending {name} to Watcher as Complete.')
    data['mode'] = 'complete'
else:
    print(f'Sending {name} to Watcher as Failed.')
    data['mode'] = 'failed'
